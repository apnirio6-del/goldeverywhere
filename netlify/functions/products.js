...
exports.handler=async(event)=>{
  ...
  try{
    if(event.httpMethod==="GET") return jsonResponse(200,await supabase("products?select=*&order=name"));
    if(event.httpMethod==="POST"){
      const rows=await supabase("products",{method:"POST",body:event.body||"{}"});
